pipeline {
    ...
    stages {
		...
		stage ('AWS Push') {
            steps {
				sh 'aws ecr get-login-password --region us-east-1 | docker login --username soumyadas755 AWS -p $(aws ecr get-login-password --region us-east-1) 796098993163.dkr.ecr.us-east-1.amazonaws.com'
